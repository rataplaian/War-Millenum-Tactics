...
export function buildAttackContext(s: GameState | undefined, id: string, target: Unit, weapon: DeepReadonly<Weapon>, choices: AttackChoices, modifier?: ModelAttackModifiers, visibility?: VisibilityProvider): AttackContext {
    ...
    return { attackerUnitId: u?.id ?? '', attackerModelId: id, targetUnitId: target.id, weapon: JSON.parse(JSON.stringify(weapon)), attackType: weapon.kind, phase: s?.phase ?? 'Shooting', turn: s?.turn ?? 1,
        ...
        targetCount: target.models.filter(m => m.alive).length, abilities: [...resolvedAbilities(s, target, weapon, choices), ...(s && u && weapon.kind==='ranged' && effectiveFlag(s,u.id,'RANGED_PISTOL') ? [{id:`blades-of-asuryan:${id}`,type:'PISTOL' as const,source:'BLADES_OF_ASURYAN'}] : []),
          ...(s && u && m && weapon.kind==='ranged' && distance<=weapon.range/2 && modelDefinition(s,u,m).abilities.some(a=>a.id==='BLADESTORM') ? [{id:`bladestorm:${id}`,type:'SUSTAINED_HITS' as const,value:1,source:'BLADESTORM'}] : [])], terrainModifiers: modifier ?? null, activeEffects: s?.flow?.effects.filter(e => e.active && e.target.unitId === u?.id).map(e => e.id) ?? [] };
}
