...
export function createTestMatch(): GameState {
  ...
  const units = PROTOTYPE_UNITS.map((definition, i) => createUnit(definition, `unit-${i + 1}`, players[i]!.id,
    Array.from({ length: definition.modelCount }, (_, j) => ({ x: 4.5 + j * 1.25, y: i === 0 ? 4.5 : 20.5 }))));
  return { schemaVersion: 1, id: 'test-battle', round: 1, turn: 1, activePlayerId: players[0].id,
