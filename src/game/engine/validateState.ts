...
export function validateState(state: GameState): void {
  if (state.schemaVersion !== 2 || !state.id || !Number.isSafeInteger(state.round) || state.round < 1 ||
      !Number.isSafeInteger(state.turn) || state.turn < 1 || !PHASES.includes(state.phase) ||
      !['in-progress', 'finished'].includes(state.status)) throw new Error('Invalid match state (schema 2 required)');
