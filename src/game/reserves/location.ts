...
export const setupBusy = (state: GameState) => !!state.reactionMove || !!state.destructionQueue?.some(q => !q.resolved && !q.waitForAttackerId) || !!state.setup || !!state.scout || !!state.transportState?.disembark || !!state.transportState?.destroyed.length || !!state.transportState?.tacticalFollowUp;
export const actionBusy = (state: GameState) => setupBusy(state) || !!state.fightOnDeath?.length || !!state.movement || !!state.shooting || !!state.closeCombat?.charge || !!state.closeCombat?.move || !!state.closeCombat?.fight?.selected;
