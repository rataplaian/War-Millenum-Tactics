...
export class StratagemEngine {
  ...
  validate(id: string, playerId: string, targets: readonly string[], mode?:string): CommandResult<StratagemDefinition> {
    ...
    for (const id of targets) {
      ...
      if (d.target.keywords?.some(k => !keywords.includes(k.toUpperCase())) || (d.target.keywordAny && !d.target.keywordAny.some(k=>keywords.includes(k.toUpperCase()))) || d.conditions?.some(c =>
