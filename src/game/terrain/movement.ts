...
export function validateTerrainPath(state: GameState, model: Model, target: Position, path?: MovementPath): CommandResult<MovementDistance> {
  ...
  for (const to of points) {
    ...
    for (const feature of features.filter(f => f.category === 'DENSE')) for (const section of feature.sections) {
      ...
      const permitted = v > EPSILON ? hasAnyKeyword(state, model, LIGHT_BODY) : hasAnyKeyword(state, model, [...LIGHT_BODY, 'MOBILE']) || section.maxZ <= rules.stepOverHeight + EPSILON;
  ...
  return { ok: true, value: { horizontal, vertical, totalMovementDistance: horizontal + vertical } };
}
